...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>{{ config('app.name', 'WiFi Manager') }} — Login</title>
                ...
                <div class="relative z-10 space-y-6">
                    <div class="space-y-3">
                        <h1 class="text-3xl font-bold leading-snug text-white xl:text-4xl">
                            WiFi Customer<br>Management
                        </h1>
                        <p class="max-w-sm text-sm leading-relaxed text-cyan-100">
                            Monitor customer payments and manage WiFi users in one simple dashboard.
                        </p>
                    </div>
                </div>
            ...
            <div class="flex min-h-screen flex-col justify-center bg-slate-100 px-6 py-10 sm:px-10">
                ...
                <div class="mx-auto w-full max-w-sm rounded-2xl border border-slate-200 bg-white p-7 shadow-xl shadow-slate-200/70 sm:p-8">
                    <div class="mb-8 space-y-1">
                        <p class="text-xs font-semibold uppercase tracking-widest text-cyan-700">Welcome Back</p>
                        <h2 class="text-2xl font-bold text-slate-900">Sign in to your dashboard</h2>
                        <p class="text-sm text-slate-500">Login to manage customers and monitor payment status.</p>
                    </div>
                    ...
                    <form method="POST" action="{{ route('login') }}" class="space-y-5">
                        ...
                        <div class="space-y-1.5">
                            <label for="login" class="text-sm font-medium text-slate-700">Email or Username</label>
                            <div class="relative">
                        ...
                        <div class="space-y-1.5">
                            <div class="flex items-center justify-between">
                                ...
                                @if (Route::has('password.request'))
                                    <a href="{{ route('password.request') }}" class="text-xs text-cyan-700 hover:text-cyan-800 hover:underline">Forgot password?</a>
                                @else
                                    <a href="#" class="text-xs text-cyan-700 hover:text-cyan-800 hover:underline">Forgot password?</a>
                                @endif
                            </div>
                            <div class="relative">
                                <span class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3 text-slate-400">
                ...
                <p class="mt-8 text-center text-xs text-slate-400">
                    Access is restricted. Please contact admin if you don't have an account.
                </p>
            </div>
        </div>
